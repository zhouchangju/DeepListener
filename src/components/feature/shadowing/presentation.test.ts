...
test("shadowing overlay avoids backdrop blur to reduce full-screen repainting", () => {
  assert.equal(
    getShadowingOverlayClassName(),
    "fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/95 p-4 outline-none sm:items-center"
  );
});

test("shadowing panel stays reachable in short windows", () => {
