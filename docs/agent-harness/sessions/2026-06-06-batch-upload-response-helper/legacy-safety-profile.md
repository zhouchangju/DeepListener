...
| Area | Paths | Notes |
|---|---|---|
| Batch upload button | `src/app/library/BatchUploadButton.tsx`, `src/app/library/BatchUploadButton.test.ts` | Delegate failed top-level batch upload response checks to the shared helper |
| Shared client response tests | `src/lib/client-response.test.ts` | Preserve helper behavior |
| Docs | `docs/agent-harness/sessions/2026-06-06-batch-upload-response-helper/**`, `CHANGELOG.md` | Record evidence only |

## Verification

| Gate | Scope | Required Result |
|---|---|---|
| targeted tests | BatchUploadButton and client-response tests | exits 0 |
