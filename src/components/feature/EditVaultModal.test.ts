...
test("edit vault modal delegates load and save response parsing to the shared helper", () => {
  ...
  assert.match(source, /requireOkResponse\(res,\s*t\("loadNoteFailed"\)\)/);
...
test("edit vault modal keeps parsed server errors visible in toasts", () => {
  ...
  assert.match(
    source,
    /catch\(\(error\) => \{\s*if \(!cancelled\) toast\.error\(error instanceof Error \? error\.message : t\("loadNoteFailed"\)\);/,
  );
  assert.match(
    source,
    /catch \(error\) \{\s*toast\.error\(error instanceof Error \? error\.message : t\("saveFailed"\)\);/,
  );
});
