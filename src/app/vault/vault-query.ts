...
type VaultListRow = Prisma.ReviewItemGetPayload<{ select: typeof vaultListSelect }>;
type VaultPlaybackRow = Prisma.ReviewItemGetPayload<{ select: typeof vaultPlaybackSelect }>;
type VaultListFindManyArgs = {
...
type VaultPlaybackFindManyArgs = {
  where: Prisma.ReviewItemWhereInput;
...
function buildVaultOrderBy(sortBy: SortOption): Prisma.ReviewItemOrderByWithRelationInput[] {
  switch (sortBy) {
    case "due":
...
function toVaultItem(row: VaultListRow, hasUserNote: boolean): VaultItem {
  return {
    ...row,
    hasUserNote,
  };
...
function toVaultPlaybackItem(row: VaultPlaybackRow): VaultPlaybackItem {
  return row;
}
