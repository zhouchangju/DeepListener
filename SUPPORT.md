...
| Packaged Windows / Linux desktop | ❌ Planned, not shipped |
...
- [ ] **Node version:** Node.js 22 or newer (CI runs on 22). Run `node -v`.
...
Windows users can use the supported Server edition from source by following the
project README. The packaged Desktop client currently supports macOS Apple
Silicon only.
